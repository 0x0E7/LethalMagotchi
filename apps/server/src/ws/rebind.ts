...
export function rebindAccountCharacter(
  deps: { hub: Hub; tournaments: TournamentService },
  accountId: string,
  ...
  for (const { connection, previousCharacterId } of changed) {
    ...
    if (previousCharacterId) deps.tournaments.onCharacterOffline(previousCharacterId);
  }
  if (characterId) deps.tournaments.onCharacterOnline(characterId);
}
